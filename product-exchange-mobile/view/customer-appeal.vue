...
import { getHistoryMessageCdToKf } from "@/request/chat";
...
export default {
  ...
  methods: {
    ...
    sendSocketMessage({ content, type }) {
      ...
      message["type"] = type;
      message["uidType"] = 4;
      message["orderId"] = this.params.orderId;
      message["orderSn"] = this.params.orderSn;
      message["nameTo"] = kfName;
      message["uidTo"] = kfId;
    ...
    getChatMessage(more = false) {
      ...
      getHistoryMessageCdToKf(params)
        .then(({ data }) => {
     ...
     */
    checkMine(item) {
      return item["uidFrom"] == this.userMember.id;
    }
  },
  mounted() {
    this.init();
